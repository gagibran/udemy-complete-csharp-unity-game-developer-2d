...
namespace Polymorphism
{
    class Program
    {
        ...
        {
            var sqlCommand = new DbCommand("SELECT * FROM Customers", new SqlConnection("MSQLS_DB", TimeSpan.FromSeconds(25)));
            sqlCommand.Execute();
            Console.WriteLine();
            var oracleCommand = new DbCommand("DELETE * FROM Customers...", new OracleConnection("ORACLE_DB", TimeSpan.FromSeconds(10)));
